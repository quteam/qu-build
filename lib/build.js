...
function getWebpackConfig() {
  ...
  if (args.compress && !args.dev && !args.watch) {
    webpackConfig.plugins = _toConsumableArray(webpackConfig.plugins).concat([new _webpack.default.optimize.UglifyJsPlugin({
      parallel: true,
      output: {
        comments: false,
        ascii_only: true
  ...
  webpackConfig.plugins = _toConsumableArray(webpackConfig.plugins).concat([new _webpack.default.NoEmitOnErrorsPlugin()]);
  ...
  if (entryArr.length > 1) {
    webpackConfig.plugins.push(new _webpack.default.optimize.CommonsChunkPlugin({
      name: 'common',
      filename: commonName
    }));
  }
